...
fn main() {
    ...
    println!("Number of painted squares: {}", number_of_painted_squares(program));
}

fn number_of_painted_squares(program: Vec<i64>) -> usize {
    ...
    // - Provide input of 0 to program (representing 0,0 black panel)
    main_output.send(0).unwrap();

    // - Wait for robot thread to halt
    // - Count the number of painted positions
