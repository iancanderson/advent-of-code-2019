...
fn wire_coordinates(wire_path: String) -> HashSet<(i32, i32)> {
...
fn closest_intersection_distance(wire1_path: String, wire2_path: String) -> i32 {
    ...
    let intersections = wire1_coordinates.intersection(&wire2_coordinates);

    // find minimum distance to (0, 0)
    return intersections.map(|coordinate|
