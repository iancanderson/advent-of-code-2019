...
fn main() {
    ...
    let mut time_steps = HashSet::new();
    loop {
        moons = time_step(&moons);

        if time_steps.contains(&moons) {
            break;
        } else {
            time_steps.insert(moons.clone());
            println!("time_steps len: {}", time_steps.len());
        }
    };

    println!("time steps: {}", time_steps.len());
}
